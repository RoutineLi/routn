...
namespace Routn{
	...
	public:
		...
		virtual bool bind(Routn::Address::ptr addr, bool ssl = false);
		virtual bool bind(const std::vector<Address::ptr>& addrs, std::vector<Address::ptr>& fail_addrs, bool ssl = false);
		virtual bool start();
		virtual void stop();
		...
		std::vector<Socket::ptr> getSocks() const { return _socks;}
